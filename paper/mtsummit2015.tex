...
The scoring system would run the training and then run the translation with the trained model.
...
For the competition, however, we focused on the theme of the hackathon, which was on unsupervised and semi-supervised models under the conditions of the oblivious setup.

In Section~\ref{sect:baseline} we outline the baseline that was used to benchmark the \ac{UNMT} in the oblivious setup.
In Section~\ref{sect:unmt} we discuss our experiments with the \ac{UNMT} model for the oblivious setup.
...
On the {\tt Lv-En} langauge pair, the model performance was mediocre.
This could be explained by the fact that {\tt En-Ru} and {\tt En-Ko} were topic-resticted corpora - both were related to tourism only, while the {\tt Lv-En} corpora was extracted from a news feed which had no topic constraints.
...
In this section we describe how the oblivious setup can be hacked to improve our results, given that the competition is structured so that the participants do not know the language-pairs being used, and it would be difficult to determine these language pairs within the scope of the competition.
...
With these information, however, one could devise at least two attacks to identify the language pair and then using this prior knowledge, use it to consturct a better translation algorithm.
