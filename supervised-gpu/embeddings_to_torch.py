...
parser.add_argument('-gpuid', type=int, default=-1, required=False,
                    help="GPU ID, or -1 for CPU")
parser.add_argument('-encoding', default='utf-8',
                    help="The econding used to read the embeddings file")
...
def get_embeddings(file):
    ...
    for l in open(file, 'rb').readlines():
        l_split = l.decode(enc).strip().split()
        if len(l_split) == 2:
            continue
...
def main():
    ...
    torch.save(filtered_enc_embeddings.cuda(device), enc_output_file)
    torch.save(filtered_dec_embeddings.cuda(device), dec_output_file)
    print("\nDone.")
